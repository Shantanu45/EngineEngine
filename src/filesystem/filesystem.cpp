...
namespace FileSystem
{
	...
	std::unique_ptr<FileMapping> File::map(std::shared_ptr<File> self)
	{
		return map_subset(0, get_size(), std::move(self));
	}

#pragma region FileMapping

	FileMapping::FileMapping(std::shared_ptr<File> handle_, uint64_t file_offset_,
...
#pragma region File System
	...
	std::unique_ptr<FileMapping> Filesystem::open_readonly_mapping(const std::string& path)
	{
		std::shared_ptr<File> file = open(path, FileMode::ReadOnly);
		if (!file) return nullptr;
		return file->map(file);
		// file destroyed here — safe because ScratchFilesystemFile/ConstantMemoryFile/FileSlice
		// pass nullptr as the FileMapping's handle, so ~FileMapping won't call unmap on it.
		// OSFilesystem mappings keep a handle ref inside the mapping itself (platform mmap).
	}

	std::unique_ptr<FileMapping> Filesystem::open_writeonly_mapping(const std::string& path, size_t size)
	{
		std::shared_ptr<File> file = open(path, FileMode::WriteOnly);
		if (!file) return nullptr;
		return file->map_write(size, file);
	}

	std::unique_ptr<FileMapping> Filesystem::open_transactional_mapping(const std::string& path, size_t size)
	{
		std::shared_ptr<File> file = open(path, FileMode::WriteOnlyTransactional);
		if (!file) return nullptr;
		return file->map_write(size, file);
	}
...
#pragma region Scratch Filesystem
	...
	{
		auto& f = scratch_files[path];
		if (!f) f = std::make_unique<ScratchFile>();
		return std::make_unique<ScratchFilesystemFile>(f->data);
	}

#pragma endregion

#pragma region Blob Filesystem

	BlobFilesystem::BlobFilesystem(std::unique_ptr<File> file_)
		: file(std::move(file_))
	{
		if (!file) return;
	...
	void BlobFilesystem::parse()
	{
		...
		auto mapped_handle = file->map(file);
		if (!mapped_handle)
	...
	{
		auto split = Path::split_no_empty(path, "/");
		auto* dir = root.get();
	...
	{
		if (mode != FileMode::ReadOnly) return nullptr;
		auto p = Path::canonicalize_path(path);
		auto* blob_file = find_file(p);
		if (!blob_file) return nullptr;
		// FileSlice holds a non-owning pointer into this->file, which is owned by BlobFilesystem.
		return std::make_unique<FileSlice>(file.get(), blob_base_offset + blob_file->offset, blob_file->size);
	}
...
#pragma region File Slice

	FileSlice::FileSlice(File* handle_, uint64_t offset_, uint64_t range_)
		: handle(handle_), offset(offset_), range(range_) {
	}

	std::unique_ptr<FileMapping> FileSlice::map_subset(uint64_t offset_, size_t range_, std::shared_ptr<File> self)
	{
		if (offset_ + range_ > range) return nullptr;
		return handle->map_subset(offset + offset_, range_, self);
	}

	std::unique_ptr<FileMapping> FileSlice::map_write(size_t, std::shared_ptr<File> self) { return nullptr; }
	...
	void FileSlice::unmap(void* mapped, size_t mapped_size) { handle->unmap(mapped, mapped_size); }
