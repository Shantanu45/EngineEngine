...
namespace Vulkan
{
	...
	static DataFormat spv_image_format_to_data_format(const SpvImageFormat p_format) {
		switch (p_format) {
		case SpvImageFormatUnknown:
			return DATA_FORMAT_MAX;
		case SpvImageFormatRgba32f:
			return DATA_FORMAT_R32G32B32A32_SFLOAT;
		case SpvImageFormatRgba16f:
			return DATA_FORMAT_R16G16B16A16_SFLOAT;
		case SpvImageFormatR32f:
			return DATA_FORMAT_R32_SFLOAT;
		case SpvImageFormatRgba8:
			return DATA_FORMAT_R8G8B8A8_UNORM;
		case SpvImageFormatRgba8Snorm:
			return DATA_FORMAT_R8G8B8A8_SNORM;
		case SpvImageFormatRg32f:
			return DATA_FORMAT_R32G32_SFLOAT;
		case SpvImageFormatRg16f:
			return DATA_FORMAT_R16G16_SFLOAT;
		case SpvImageFormatR11fG11fB10f:
			return DATA_FORMAT_B10G11R11_UFLOAT_PACK32;
		case SpvImageFormatR16f:
			return DATA_FORMAT_R16_SFLOAT;
		case SpvImageFormatRgba16:
			return DATA_FORMAT_R16G16B16A16_UNORM;
		case SpvImageFormatRgb10A2:
			return DATA_FORMAT_A2B10G10R10_UNORM_PACK32;
		case SpvImageFormatRg16:
			return DATA_FORMAT_R16G16_UNORM;
		case SpvImageFormatRg8:
			return DATA_FORMAT_R8G8_UNORM;
		case SpvImageFormatR16:
			return DATA_FORMAT_R16_UNORM;
		case SpvImageFormatR8:
			return DATA_FORMAT_R8_UNORM;
		case SpvImageFormatRgba16Snorm:
			return DATA_FORMAT_R16G16B16A16_SNORM;
		case SpvImageFormatRg16Snorm:
			return DATA_FORMAT_R16G16_SNORM;
		case SpvImageFormatRg8Snorm:
			return DATA_FORMAT_R8G8_SNORM;
		case SpvImageFormatR16Snorm:
			return DATA_FORMAT_R16_SNORM;
		case SpvImageFormatR8Snorm:
			return DATA_FORMAT_R8_SNORM;
		case SpvImageFormatRgba32i:
			return DATA_FORMAT_R32G32B32A32_SINT;
		case SpvImageFormatRgba16i:
			return DATA_FORMAT_R16G16B16A16_SINT;
		case SpvImageFormatRgba8i:
			return DATA_FORMAT_R8G8B8A8_SINT;
		case SpvImageFormatR32i:
			return DATA_FORMAT_R32_SINT;
		case SpvImageFormatRg32i:
			return DATA_FORMAT_R32G32_SINT;
		case SpvImageFormatRg16i:
			return DATA_FORMAT_R16G16_SINT;
		case SpvImageFormatRg8i:
			return DATA_FORMAT_R8G8_SINT;
		case SpvImageFormatR16i:
			return DATA_FORMAT_R16_SINT;
		case SpvImageFormatR8i:
			return DATA_FORMAT_R8_SINT;
		case SpvImageFormatRgba32ui:
			return DATA_FORMAT_R32G32B32A32_UINT;
		case SpvImageFormatRgba16ui:
			return DATA_FORMAT_R16G16B16A16_UINT;
		case SpvImageFormatRgba8ui:
			return DATA_FORMAT_R8G8B8A8_UINT;
		case SpvImageFormatR32ui:
			return DATA_FORMAT_R32_UINT;
		case SpvImageFormatRgb10a2ui:
			return DATA_FORMAT_A2B10G10R10_UINT_PACK32;
		case SpvImageFormatRg32ui:
			return DATA_FORMAT_R32G32_UINT;
		case SpvImageFormatRg16ui:
			return DATA_FORMAT_R16G16_UINT;
		case SpvImageFormatRg8ui:
			return DATA_FORMAT_R8G8_UINT;
		case SpvImageFormatR16ui:
			return DATA_FORMAT_R16_UINT;
		case SpvImageFormatR8ui:
			return DATA_FORMAT_R8_UINT;
		case SpvImageFormatR64ui:
			return DATA_FORMAT_R64_UINT;
		case SpvImageFormatR64i:
			return DATA_FORMAT_R64_SINT;
	...
	static PipelineSpecializationConstantType spv_spec_constant_type_from_id(const SpvReflectShaderModule& module, uint32_t spirv_id)
	{
		uint32_t type_id = 0;
			...
			for (size_t t = 0; t < module._internal->type_description_count; ++t) {
				...
				if (td->id == type_id) {
					...
					case SPV_REFLECT_TYPE_FLAG_BOOL:
						...
						return PIPELINE_SPECIALIZATION_CONSTANT_TYPE_BOOL;
						break;
					case SPV_REFLECT_TYPE_FLAG_INT:
						return PIPELINE_SPECIALIZATION_CONSTANT_TYPE_INT;
						break;
					case SPV_REFLECT_TYPE_FLAG_FLOAT:
						return 	PIPELINE_SPECIALIZATION_CONSTANT_TYPE_FLOAT;
						break;
					default:
						break;
					}
	...
	ShaderReflection RenderingShaderContainer::get_shader_reflection() const
	{
		...
		for (uint32_t i = 0; i < reflection_data.set_count; i++) {
			std::vector<ShaderUniform>& uniform_set = shader_refl.uniform_sets.data()[i];
			...
			for (uint32_t j = 0; j < uniforms_count; j++) {
				...
				ShaderUniform& uniform = uniform_set.data()[j];
				uniform.type = UniformType(binding.type);
		...
		for (uint32_t i = 0; i < reflection_data.specialization_constants_count; i++) {
			...
			ShaderSpecializationConstant& sc = shader_refl.specialization_constants.data()[i];
			sc.type = PipelineSpecializationConstantType(spec.type);
		...
		for (uint32_t i = 0; i < reflection_data.stage_count; i++) {
			...
			shader_refl.stages_bits.set_flag(ShaderStage(1U << reflection_shader_stages[i]));
		}
		...
		for (uint32_t i = 0; i < spirv_size; i++) {
			...
			if (!pipeline_type_detected) {
				...
				case SHADER_STAGE_TESSELATION_EVALUATION:
					r_shader.pipeline_type = PIPELINE_TYPE_RASTERIZATION;
					break;
				case SHADER_STAGE_COMPUTE:
					r_shader.pipeline_type = PIPELINE_TYPE_COMPUTE;
					break;
				...
				case SHADER_STAGE_INTERSECTION:
					r_shader.pipeline_type = PIPELINE_TYPE_RAYTRACING;
					break;
				default:
			...
			ERR_FAIL_COND_V_MSG(reflection.stages_bits.has_flag(stage_flag), FAILED,
				std::format("Stage %s submitted more than once.", std::string(SHADER_STAGE_NAMES[stage])));
...

			{
				...
				ERR_FAIL_COND_V_MSG(result != SPV_REFLECT_RESULT_SUCCESS, FAILED,
					"Reflection of SPIR-V shader stage '" + std::string(SHADER_STAGE_NAMES[p_spirv[i].shader_stage]) + "' failed parsing shader.");
				...
				ERR_FAIL_COND_V_MSG(result != SPV_REFLECT_RESULT_SUCCESS, FAILED,
					"Reflection of SPIR-V shader stage '" + std::string(SHADER_STAGE_NAMES[p_spirv[i].shader_stage]) + "' failed enumerating descriptor bindings.");
				...
				if (binding_count > 0) {
					...
					ERR_FAIL_COND_V_MSG(result != SPV_REFLECT_RESULT_SUCCESS, FAILED,
						"Reflection of SPIR-V shader stage '" + std::string(SHADER_STAGE_NAMES[p_spirv[i].shader_stage]) + "' failed getting descriptor bindings.");
...

				{
					...
					ERR_FAIL_COND_V_MSG(result != SPV_REFLECT_RESULT_SUCCESS, FAILED,
						"Reflection of SPIR-V shader stage '" + std::string(SHADER_STAGE_NAMES[p_spirv[i].shader_stage]) + "' failed enumerating specialization constants.");
					...
					if (sc_count) {
						...
						ERR_FAIL_COND_V_MSG(result != SPV_REFLECT_RESULT_SUCCESS, FAILED,
							"Reflection of SPIR-V shader stage '" + std::string(SHADER_STAGE_NAMES[p_spirv[i].shader_stage]) + "' failed obtaining specialization constants.");
				...
				if (stage == SHADER_STAGE_VERTEX || stage == SHADER_STAGE_FRAGMENT) {
					...
					ERR_FAIL_COND_V_MSG(result != SPV_REFLECT_RESULT_SUCCESS, FAILED,
						"Reflection of SPIR-V shader stage '" + std::string(SHADER_STAGE_NAMES[p_spirv[i].shader_stage]) + "' failed enumerating input variables.");
					...
					if (iv_count) {
						...
						ERR_FAIL_COND_V_MSG(result != SPV_REFLECT_RESULT_SUCCESS, FAILED,
							"Reflection of SPIR-V shader stage '" + std::string(SHADER_STAGE_NAMES[p_spirv[i].shader_stage]) + "' failed obtaining input variables.");
				...
				if (stage == SHADER_STAGE_FRAGMENT) {
					...
					ERR_FAIL_COND_V_MSG(result != SPV_REFLECT_RESULT_SUCCESS, FAILED,
						"Reflection of SPIR-V shader stage '" + std::string(SHADER_STAGE_NAMES[p_spirv[i].shader_stage]) + "' failed enumerating output variables.");
					...
					if (ov_count) {
						...
						ERR_FAIL_COND_V_MSG(result != SPV_REFLECT_RESULT_SUCCESS, FAILED,
							"Reflection of SPIR-V shader stage '" + std::string(SHADER_STAGE_NAMES[p_spirv[i].shader_stage]) + "' failed obtaining output variables.");
				...
				ERR_FAIL_COND_V_MSG(result != SPV_REFLECT_RESULT_SUCCESS, FAILED,
					"Reflection of SPIR-V shader stage '" + std::string(SHADER_STAGE_NAMES[p_spirv[i].shader_stage]) + "' failed enumerating push constants.");

				if (pc_count) {
					ERR_FAIL_COND_V_MSG(pc_count > 1, FAILED,
						"Reflection of SPIR-V shader stage '" + std::string(SHADER_STAGE_NAMES[p_spirv[i].shader_stage]) + "': Only one push constant is supported, which should be the same across shader stages.");
					...
					ERR_FAIL_COND_V_MSG(result != SPV_REFLECT_RESULT_SUCCESS, FAILED,
						"Reflection of SPIR-V shader stage '" + std::string(SHADER_STAGE_NAMES[p_spirv[i].shader_stage]) + "' failed obtaining push constants.");
#if 0
					...
					ERR_FAIL_COND_V_MSG(reflection.push_constant_size && reflection.push_constant_size != pconstants[0]->size, FAILED,
						"Reflection of SPIR-V shader stage '" + std::string(SHADER_STAGE_NAMES[p_spirv[i].shader_stage]) + "': Push constant block must be the same across shader stages.");
