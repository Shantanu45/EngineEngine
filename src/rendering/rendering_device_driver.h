...
#define DEFINE_ID(m_name) \
	...
	public:
		...
		enum BufferUsageBits {
			...
			BUFFER_USAGE_STORAGE_TEXEL_BIT = (1 << 3),
		...
		virtual uint64_t buffer_get_dynamic_offsets(std::span<BufferID> p_buffers) = 0;
		...
		virtual VertexFormatID vertex_format_create(std::span<VertexAttribute> p_vertex_attribs, const VertexAttributeBindingsMap& p_vertex_bindings) = 0;
		...
		virtual void command_pipeline_barrier(
			...
			std::span<AccelerationStructureBarrier> p_acceleration_structure_barriers) = 0;
		...
		virtual Error command_queue_execute_and_present(CommandQueueID p_cmd_queue, std::span<SemaphoreID> p_wait_semaphores, std::span<CommandBufferID> p_cmd_buffers, std::span<SemaphoreID> p_cmd_semaphores, FenceID p_cmd_fence, std::span<SwapChainID> p_swap_chains) = 0;
		...
		virtual void command_buffer_execute_secondary(CommandBufferID p_cmd_buffer, std::span<CommandBufferID> p_secondary_cmd_buffers) = 0;
	...
	public:
		...
		virtual UniformSetID uniform_set_create(std::span<BoundUniform> p_uniforms, ShaderID p_shader, uint32_t p_set_index, int p_linear_pool_index) = 0;
		...
		virtual uint32_t uniform_sets_get_dynamic_offsets(std::span<UniformSetID> p_uniform_sets, ShaderID p_shader, uint32_t p_first_set_index, uint32_t p_set_count) const = 0;
		...
		virtual void command_copy_buffer(CommandBufferID p_cmd_buffer, BufferID p_src_buffer, BufferID p_dst_buffer, std::span<BufferCopyRegion> p_regions) = 0;

		virtual void command_copy_texture(CommandBufferID p_cmd_buffer, TextureID p_src_texture, TextureLayout p_src_texture_layout, TextureID p_dst_texture, TextureLayout p_dst_texture_layout, std::span<TextureCopyRegion> p_regions) = 0;
		...
		virtual void command_copy_buffer_to_texture(CommandBufferID p_cmd_buffer, BufferID p_src_buffer, TextureID p_dst_texture, TextureLayout p_dst_texture_layout, std::span<BufferTextureCopyRegion> p_regions) = 0;
		virtual void command_copy_texture_to_buffer(CommandBufferID p_cmd_buffer, TextureID p_src_texture, TextureLayout p_src_texture_layout, BufferID p_dst_buffer, std::span<BufferTextureCopyRegion> p_regions) = 0;
		...
		virtual void command_bind_push_constants(CommandBufferID p_cmd_buffer, ShaderID p_shader, uint32_t p_first_index, std::span<uint32_t> p_data) = 0;
		...
		virtual RenderPassID render_pass_create(std::span<Attachment> p_attachments, std::span<Subpass> p_subpasses, std::span<SubpassDependency> p_subpass_dependencies, uint32_t p_view_count, AttachmentReference p_fragment_density_map_attachment) = 0;
		...
		virtual void command_begin_render_pass(CommandBufferID p_cmd_buffer, RenderPassID p_render_pass, FramebufferID p_framebuffer, CommandBufferType p_cmd_buffer_type, const Rect2i& p_rect, std::span<RenderPassClearValue> p_clear_values) = 0;
		...
		virtual void command_render_set_viewport(CommandBufferID p_cmd_buffer, std::span<Rect2i> p_viewports) = 0;
		virtual void command_render_set_scissor(CommandBufferID p_cmd_buffer, std::span<Rect2i> p_scissors) = 0;
		virtual void command_render_clear_attachments(CommandBufferID p_cmd_buffer, std::span<AttachmentClear> p_attachment_clears, std::span<Rect2i> p_rects) = 0;

		// Binding.
		virtual void command_bind_render_pipeline(CommandBufferID p_cmd_buffer, PipelineID p_pipeline) = 0;
		virtual void command_bind_render_uniform_sets(CommandBufferID p_cmd_buffer, std::span<UniformSetID> p_uniform_sets, ShaderID p_shader, uint32_t p_first_set_index, uint32_t p_set_count, uint32_t p_dynamic_offsets) = 0;
		...
		virtual PipelineID render_pipeline_create(
			...
			std::span<PipelineSpecializationConstant> p_specialization_constants) = 0;
		...
		//virtual void tlas_instances_buffer_fill(BufferID p_instances_buffer, std::span<AccelerationStructureID> p_blases, std::span<Transform3D> p_transforms) = 0;
