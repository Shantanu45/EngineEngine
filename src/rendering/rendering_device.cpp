...
namespace Rendering
{
	...
	Rendering::RenderingDevice::FramebufferFormatID RenderingDevice::framebuffer_format_create(const std::vector<AttachmentFormat>& p_format, uint32_t p_view_count /*= 1*/, int32_t p_fragment_density_map_attachment /*= -1*/)
	{
	...
	Rendering::RenderingDevice::FramebufferFormatID RenderingDevice::framebuffer_format_create_multipass(const std::vector<AttachmentFormat>& p_attachments, const std::vector<FramebufferPass>& p_passes, uint32_t p_view_count /*= 1*/, int32_t p_vrs_attachment /*= -1*/)
	{
	...
	RDD::RenderPassID RenderingDevice::_render_pass_create(RenderingDeviceDriver* p_driver, const std::vector<AttachmentFormat>& p_attachments, const std::vector<FramebufferPass>& p_passes, std::span<RDD::AttachmentLoadOp> p_load_ops, std::span<RDD::AttachmentStoreOp> p_store_ops, uint32_t p_view_count /*= 1*/, VRSMethod p_vrs_method /*= VRS_METHOD_NONE*/, int32_t p_vrs_attachment /*= -1*/, Size2i p_vrs_texel_size /*= Size2i()*/, std::vector<TextureSamples>* r_samples /*= nullptr*/)
	{
		// NOTE:
	...
	// to the new architecture.
		...
		for (int i = 0; i < p_attachments.size(); i++) {
			...
			ERR_FAIL_COND_V_MSG(!(p_attachments[i].usage_flags & (TEXTURE_USAGE_COLOR_ATTACHMENT_BIT | TEXTURE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT | TEXTURE_USAGE_DEPTH_RESOLVE_ATTACHMENT_BIT | TEXTURE_USAGE_INPUT_ATTACHMENT_BIT | TEXTURE_USAGE_VRS_ATTACHMENT_BIT)),
				RDD::RenderPassID(), "Texture format for index (%s) requires an attachment (color, depth-stencil, input or VRS) bit set.", std::to_string(i));
		...
		for (int i = 0; i < p_passes.size(); i++) {
			...
			for (int j = 0; j < pass->color_attachments.size(); j++) {
				...
				if (attachment == ATTACHMENT_UNUSED) {
					...
					ERR_FAIL_INDEX_V_MSG(attachment, p_attachments.size(), RDD::RenderPassID(), "Invalid framebuffer format attachment( %s ), in pass (%s), color attachment (%s).", std::to_string(attachment), std::to_string(i), std::to_string(j));
					ERR_FAIL_COND_V_MSG(!(p_attachments[attachment].usage_flags & TEXTURE_USAGE_COLOR_ATTACHMENT_BIT), RDD::RenderPassID(), "Invalid framebuffer format attachment(%s), in pass (%s), it's marked as depth, but it's not usable as color attachment.", std::to_string(attachment), std::to_string(i));
					ERR_FAIL_COND_V_MSG(attachment_last_pass[attachment] == i, RDD::RenderPassID(), "Invalid framebuffer format attachment(%s), in pass (%s), it already was used for something else before in this pass.", std::to_string(attachment), std::to_string(i));
					...
					if (is_multisample_first) {
						...
						ERR_FAIL_COND_V_MSG(texture_samples != p_attachments[attachment].samples, RDD::RenderPassID(), "Invalid framebuffer format attachment(%s), in pass (%s), if an attachment is marked as multisample, all of them should be multisample and use the same number of samples.", std::to_string(attachment), std::to_string(i));
					}
			...
			for (int j = 0; j < pass->input_attachments.size(); j++) {
				...
				if (attachment == ATTACHMENT_UNUSED) {
					...
					ERR_FAIL_INDEX_V_MSG(attachment, p_attachments.size(), RDD::RenderPassID(), "Invalid framebuffer format attachment(%s), in pass (%s), input attachment (%s).", std::to_string(attachment), std::to_string(i), sd::to_string(j));
					ERR_FAIL_COND_V_MSG(!(p_attachments[attachment].usage_flags & TEXTURE_USAGE_INPUT_ATTACHMENT_BIT), RDD::RenderPassID(), "Invalid framebuffer format attachment(%s), in pass (%s), it isn't marked as an input texture.", std::to_string(attachment), std::to_string(i));
					ERR_FAIL_COND_V_MSG(attachment_last_pass[attachment] == i, RDD::RenderPassID(), "Invalid framebuffer format attachment(%s), in pass (%s), it already was used for something else before in this pass.", std::to_string(attachment), std::to_string(i));
					reference.attachment = attachment_remap[attachment];
			...
			if (pass->resolve_attachments.size() > 0) {
				ERR_FAIL_COND_V_MSG(pass->resolve_attachments.size() != pass->color_attachments.size(), RDD::RenderPassID(), "The amount of resolve attachments (%s) must match the number of color attachments (%s).", std::to_string(pass->resolve_attachments.size()), std::to_string(pass->color_attachments.size()));
			...
			for (int j = 0; j < pass->resolve_attachments.size(); j++) {
				...
				if (attachment == ATTACHMENT_UNUSED) {
					...
					ERR_FAIL_INDEX_V_MSG(attachment, p_attachments.size(), RDD::RenderPassID(), "Invalid framebuffer format attachment(%s), in pass (%s), resolve attachment (%s).", std::to_string(attachment), std::to_string(i), std::to_string(j));
					ERR_FAIL_COND_V_MSG(pass->color_attachments[j] == ATTACHMENT_UNUSED, RDD::RenderPassID(), "Invalid framebuffer format attachment(%s), in pass (%s), resolve attachment (%s), the respective color attachment is marked as unused.", std::to_string(attachment), std::to_string(i), std::to_string(j));
					ERR_FAIL_COND_V_MSG(!(p_attachments[attachment].usage_flags& TEXTURE_USAGE_COLOR_ATTACHMENT_BIT), RDD::RenderPassID(), "Invalid framebuffer format attachment(%s), in pass (%s), resolve attachment, it isn't marked as a color texture.", std::to_string(attachment), std::to_string(i));
					ERR_FAIL_COND_V_MSG(attachment_last_pass[attachment] == i, RDD::RenderPassID(), "Invalid framebuffer format attachment(%s), in pass (%s), it already was used for something else before in this pass.", std::to_string(attachment), std::to_string(i));
					bool multisample = p_attachments[attachment].samples > TEXTURE_SAMPLES_1;
					ERR_FAIL_COND_V_MSG(multisample, RDD::RenderPassID(), "Invalid framebuffer format attachment(%s), in pass (%s), resolve attachments can't be multisample.", std::to_string(attachment), std::to_string(i));
					reference.attachment = attachment_remap[attachment];
			...
			if (pass->depth_attachment != ATTACHMENT_UNUSED) {
				...
				ERR_FAIL_INDEX_V_MSG(attachment, p_attachments.size(), RDD::RenderPassID(), "Invalid framebuffer depth format attachment(%s), in pass (%s), depth attachment.", std::to_string(attachment), std::to_string(i));
				ERR_FAIL_COND_V_MSG(!(p_attachments[attachment].usage_flags& TEXTURE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT), RDD::RenderPassID(), "Invalid framebuffer depth format attachment(%s), in pass (%s), it's marked as depth, but it's not a depth attachment.", std::to_string(attachment), std::to_string(i));
				ERR_FAIL_COND_V_MSG(attachment_last_pass[attachment] == i, RDD::RenderPassID(), "Invalid framebuffer depth format attachment(%s), in pass (%s), it already was used for something else before in this pass.", std::to_string(attachment), std::to_string(i));
				...
				if (is_multisample_first) {
					...
					ERR_FAIL_COND_V_MSG(texture_samples != p_attachments[attachment].samples, RDD::RenderPassID(), "Invalid framebuffer depth format attachment(%s), in pass (%s), if an attachment is marked as multisample, all of them should be multisample and use the same number of samples including the depth.", std::to_string(attachment), std::to_string(i));
				}
				...
				if (pass->depth_resolve_attachment != ATTACHMENT_UNUSED) {
					...
					ERR_FAIL_COND_V_MSG(!p_driver->has_feature(SUPPORTS_FRAMEBUFFER_DEPTH_RESOLVE), RDD::RenderPassID(), "Invalid framebuffer depth format attachment(%s), in pass (%s), a depth resolve attachment was supplied when driver doesn't support this feature.", std::to_string(attachment), std::to_string(i));

					ERR_FAIL_INDEX_V_MSG(attachment, p_attachments.size(), RDD::RenderPassID(), "Invalid framebuffer depth resolve format attachment(%s), in pass (%s), depth resolve attachment.", std::to_string(attachment), std::to_string(i));
					ERR_FAIL_COND_V_MSG(!(p_attachments[attachment].usage_flags & TEXTURE_USAGE_DEPTH_RESOLVE_ATTACHMENT_BIT), RDD::RenderPassID(), "Invalid framebuffer depth resolve format attachment(%s), in pass (%s), it's marked as depth, but it's not a depth resolve attachment.", std::to_string(attachment), std::to_string(i));
					ERR_FAIL_COND_V_MSG(attachment_last_pass[attachment] == i, RDD::RenderPassID(), "Invalid framebuffer depth resolve format attachment(%s), in pass (%s), it already was used for something else before in this pass.", std::to_string(attachment), std::to_string(i));
			...
			if (p_vrs_method == VRS_METHOD_FRAGMENT_SHADING_RATE && p_vrs_attachment >= 0) {
				...
				ERR_FAIL_INDEX_V_MSG(attachment, p_attachments.size(), RDD::RenderPassID(), "Invalid framebuffer VRS format attachment(%s), in pass (%s), VRS attachment.", std::to_string(attachment), std::to_string(i));
				ERR_FAIL_COND_V_MSG(!(p_attachments[attachment].usage_flags& TEXTURE_USAGE_VRS_ATTACHMENT_BIT), RDD::RenderPassID(), "Invalid framebuffer VRS format attachment(%s), in pass (%s), it's marked as VRS, but it's not a VRS attachment.", std::to_string(attachment), std::to_string(i));
				ERR_FAIL_COND_V_MSG(attachment_last_pass[attachment] == i, RDD::RenderPassID(), "Invalid framebuffer VRS attachment(%s), in pass (%s), it already was used for something else before in this pass.", std::to_string(attachment), std::to_string(i));
			...
			for (int j = 0; j < pass->preserve_attachments.size(); j++) {
				...
				ERR_FAIL_COND_V_MSG(attachment == ATTACHMENT_UNUSED, RDD::RenderPassID(), "Invalid framebuffer format attachment(%s), in pass (%s), preserve attachment (%s). Preserve attachments can't be unused.", std::to_string(attachment), std::to_string(i), std::to_string(j));

				ERR_FAIL_INDEX_V_MSG(attachment, p_attachments.size(), RDD::RenderPassID(), "Invalid framebuffer format attachment(%s), in pass (%s), preserve attachment (%s).", std::to_string(attachment), std::to_string(i), std::to_string(j));
		...
		const std::vector<PipelineSpecializationConstant>& p_specialization_constants /*= std::vector<PipelineSpecializationConstant>()*/)
	{
		...
		{
			ERR_FAIL_COND_V_MSG(p_for_render_pass >= uint32_t(fb_format.E->first.passes.size()), RID(), "Render pass requested for pipeline creation (%s) is out of bounds", std::to_string(p_for_render_pass));
			...
			ERR_FAIL_COND_V_MSG(shader->fragment_output_mask != output_mask, RID(),
				"Mismatch fragment shader output mask (%s) and framebuffer color output mask (%s) when binding both in render pipeline.", std::to_string(shader->fragment_output_mask), std::to_string(output_mask));
		}
		...
		if (p_vertex_format != INVALID_ID) {
			...
			for (uint32_t i = 0; i < 64; i++) {
				...
				ERR_FAIL_COND_V_MSG(!found, RID(),
					"Shader vertex input location (%s) not provided in vertex input description for pipeline creation.", std::to_string(i));
			}
		...
		for (int i = 0; i < shader->specialization_constants.size(); i++) {
			...
			for (int j = 0; j < p_specialization_constants.size(); j++) {
				...
				if (psc.constant_id == sc.constant_id) {
					ERR_FAIL_COND_V_MSG(psc.type != sc.type, RID(), "Specialization constant provided for id (%s) is of the wrong type.", std::to_string(sc.constant_id));
					break;
				}
			}
		}
