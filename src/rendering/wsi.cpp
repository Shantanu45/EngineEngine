...
namespace Rendering
{
	...
	void WSI::blit_initialize()
	{
		...
		blit.array = rendering_device->index_array_create(blit.index_buffer, 0, 3);
	...
	void WSI::blit_render_target_to_screen(DisplayServerEnums::WindowID p_screen, const BlitToScreen* p_render_targets)
	{
		...
		//rendering_device->render_draw_indexed(rendering_device->get_current_command_buffer(), 3, 1, 0, 0, 0);
		rendering_device->render_draw(rendering_device->get_current_command_buffer(), 3, 1);
	}
