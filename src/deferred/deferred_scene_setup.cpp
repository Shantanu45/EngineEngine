...
void add_default_deferred_lights(entt::registry& world, DeferredDemoLightMeshes meshes)
{
...
DeferredSceneHandle instantiate_deferred_gltf_scene(const DeferredGltfSceneRequest& request)
{
	DeferredSceneHandle handle;

	Rendering::MeshLoader mesh_loader(request.filesystem, request.device);
	if (!mesh_loader.load_file(request.path)) {
		LOGE("instantiate_deferred_gltf_scene: failed to load '{}'", request.path);
		return handle;
	}
	...
	auto imported_scene = gltf_scene_importer.import(Rendering::GltfSceneImportContext{
		...
		.fallback_textures = request.fallback_textures,
		...
		.transparent_shader_rid = request.transparent_shader_rid,
		.transparent_pbr_shader_rid = request.transparent_pbr_shader_rid,
