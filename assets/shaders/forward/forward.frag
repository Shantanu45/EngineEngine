...
void main()
{
    ...
    for (uint i = 0u; i < lightData.lightCount; i++) {
        ...
        if (lightData.lights[i].type == LIGHT_DIRECTIONAL)
            color += shadow * CalcDirLight(lightData.lights[i], baseColor.rgb, metallic, roughness, normal, viewDir);
        else if (lightData.lights[i].type == LIGHT_POINT)
            color += shadow * CalcPointLight(lightData.lights[i], baseColor.rgb, metallic, roughness, normal, FragPos, viewDir);
        else if (lightData.lights[i].type == LIGHT_SPOT)
            color += shadow * CalcSpotLight(lightData.lights[i], baseColor.rgb, metallic, roughness, normal, FragPos, viewDir);
    }
